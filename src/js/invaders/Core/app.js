...
App.prototype = {
    ...
    World: {
        draw: function () {
            ...
            Game.canvas.fillStyle = top > 0 ? '#00FF00' : '#FFFFFF';
            // Draw the Score
            Game.canvas.fillText(
                Game.Score.local.score,
                Game.border,
                20
            );
            // Draw the Initials
            initialsSize = Game.canvas.measureText(Game.Score.local.initials).width;
            ...
            } else if (top) { // Otherwise draw if the user is currently placing in the high scores
                Game.canvas.fillText(
                    Game.Score.GetPlacing(),
                    Game.width - (Game.width - Game.border),
                    Game.height - Game.border * 2
                );
            }
        }
    },
